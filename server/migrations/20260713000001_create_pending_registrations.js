...
// registrant's email is verified via OTP. This table holds the submitted
...
exports.up = async function (knex) {
  await knex.schema.createTable("pending_registrations", (t) => {
    ...
    t.string("phone", 32); // optional, unverified contact info — not the verification channel
  ...
  // Only one *live* (unconsumed) pending registration per email at a time,
  // so retries update the same lineage instead of piling up rows, and two
  // concurrent registration attempts for the same email can't both stay
  // pending forever.
  await knex.raw(`
